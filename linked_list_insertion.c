#include <stdio.h>
#include <stdlib.h>
// ADT for the Sinlgy Linked List.
struct node
{
    int data;
    struct node *next;
};
// Traversal method to access all the elements of the Linked List.
...
struct node *insertionat_afternode(struct node *a, struct node *prevnode, int data)
{
    struct node *e;
    e = (struct node *)malloc(sizeof(struct node));
    (*e).data = data;
    (*e).next = (*prevnode).next;
    (*prevnode).next = e;
...
int main(void)
{
    ...
    int m;
    printf("Enter the element that you want to insert after second node :\n");
    scanf("%d", &m);
    struct node *a = (*head_node).next;
    head_node = insertionat_afternode(head_node, a, m);
    traversallinked_list(head_node);
