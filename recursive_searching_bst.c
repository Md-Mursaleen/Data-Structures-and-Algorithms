#include <stdio.h>
#include <stdlib.h>
// Program to search an element in a BST (Recursive Searching opeartion in a BST).
struct node
{
    int data;
    struct node *left;
...
struct node *recur_search_BST(struct node *a, int key)
{
    if (a == NULL)
    {
        return NULL;
    ...
    {
        return recur_search_BST((*a).left, key);
    }
    else
    {
        return recur_search_BST((*a).right, key);
    }
}
int main(void)
{
    ...
    // Recursive Searching of the above BST is:
    int key;
    printf("\nEnter the element that you want to search in this BST : ");
    scanf("%d", &key);
    if (recur_search_BST(r, key) != NULL)
    {
        printf("\nThe element is found in this BST");
    }
    else
    {
